...
export async function sendEmailWithTemplate(input: SendEmailInput): Promise<SendEmailResult> {
  ...
  const followUpId = input.followUpId ? String(input.followUpId).trim() : null;

  if (!isUuid(clinicId) || !isUuid(templateId) || (contactId && !isUuid(contactId)) || (followUpId && !isUuid(followUpId))) {
    return { ok: false, status: 400, error: "Invalid identifiers supplied" };
  }
  ...
  if (clinicError || !clinic) {
    return {
      ...
      error: clinicError ? `Clinic lookup failed: ${clinicError.message}` : "Clinic not found for owner",
    };
  }

  if (templateError || !template) {
    return { ok: false, status: 404, error: "Template not found for owner" };
  }
  ...
  if (contactId) {
    ...
    if (contactError || !contactData) {
      return { ok: false, status: 404, error: "Contact not found for owner/clinic" };
    }
  ...
  if (!toAddress || !toAddress.includes("@")) {
    return { ok: false, status: 400, error: "Recipient email is missing or invalid" };
  }
  ...
  try {
    if (isFirstContact) {
      ...
      let followUpTask: { id: string; status: string } | null = null;

      if (followUpId) {
        const { data: specificFollowUpTask, error: specificFollowUpLookupError } = await supabaseAdmin
          .from("follow_ups")
          .select("id, status")
          .eq("owner_id", CRM_OWNER_ID)
          .eq("clinic_id", clinicId)
        ...
        if (specificFollowUpLookupError) {
          throw specificFollowUpLookupError;
        }

        if (specificFollowUpTask) {
          followUpTask = {
            id: String(specificFollowUpTask.id),
            status: String(specificFollowUpTask.status || ""),
          };
        }
      } else {
        const { data: queuedFollowUpTask, error: followUpLookupError } = await supabaseAdmin
          .from("follow_ups")
          .select("id, status")
          .eq("owner_id", CRM_OWNER_ID)
          .eq("clinic_id", clinicId)
        ...
        if (queuedFollowUpTask) {
          followUpTask = {
            id: String(queuedFollowUpTask.id),
            status: String(queuedFollowUpTask.status || ""),
          };
        }
      }
